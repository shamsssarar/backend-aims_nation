...
const createWeeklyReportSchema = z.object({
  ...
  weekStartDate: z.string('Week start date is required').datetime(), // Must be an ISO date string
  daysPresent: z.number().int().min(0),
  daysAbsent: z.number().int().min(0),
  examScore: z.number().min(0).max(100).optional(),
  behaviorStatus: z.enum(['EXCEPTIONAL', 'NORMAL', 'NEEDS_ATTENTION']).optional(),
